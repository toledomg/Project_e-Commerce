...
function boxMsg() {
  let textoMsg = document.createElement("p");
  textoMsg.innerText = `Carrinho vazio
		Adicione Itens`;
  textoMsg.id = "textoMsg";
  cartContainer.classList = "productsCart emptyCart";
  cartContainer.appendChild(textoMsg);
}
...
//
function contaProduto(item) {
  for (let i = 0; i < item.length; i++) {
    let contarBtn = botoesProduto[i];
    contarBtn.addEventListener("click", function (e) {
      let idProduto = e.target.id;
      let id = parseInt(idProduto.substring(5));
      if (id == id) {
        itemSoma++;
        document.querySelector("#quant_Produto").innerHTML = `${itemSoma}`;
      }
      console.log(id);
    });
  }
...
function filterText() {
  let buscar = pesquisaBox.value.toLowerCase();
  let ul = document.getElementById("itens__produto__ul");
  ul.remove();

  let filtroLista = data.filter((e) => {
    let buscarTitulo = e.title.toLowerCase();
    let buscarCategoria = e.category.toLowerCase();
    if (
      buscarCategoria.includes(buscar) |
      (buscar == "todos") |
      buscarTitulo.includes(buscar)
    ) {
      return true;
    } else {
      return false;
